...
class Volume extends BaseCommand {
    ...
    get arguments() {
        return [{ name: "command_name", description: "The command you want to toggle. If this command is not in the list, it will be added, else it will be removed!", required: !0 }];
    }
    async run({ ctx: e }) {
        if ("reset" === e.args[0])
            return (
        ...
        const s = e.client.commmands.commands.get(e.args[0]) || e.client.commmands.commands.get((e) => e.aliases && e.aliases.includes(e.args[0]));
        ...
        e.guildDB.dj_commands.includes(`${s.name}`) ?
            ((e.guildDB.dj_commands = e.guildDB.dj_commands.filter((e) => e !== `${s.name}`)), e.client.mongoDB.handleCache(e.guildDB), e.successMessage(`The \`${s.name}\` command is no longer a DJ command!`)) :
