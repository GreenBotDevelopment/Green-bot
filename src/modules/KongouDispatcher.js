class KongouDispatcher {
    constructor({ client: t, metadata: e, player: i, node: s }) {
        i &&
            t &&
            ((this.client = t),
            (this.metadata = e),
                    ...
                    embeds: [
                        {
                            ...
                            author: {
                                name: "Track errored",
                                icon_url: this.metadata.guild.icon ? this.metadata.guild.iconURL({ dynamic: !0 }) : "https://cdn.discordapp.com/attachments/748897191879245834/782271474450825226/0.png?size=128",
                            },
                          ...
                          embeds: [
                              {
                                  ...
                                  footer: { text: `${this.queue.length} songs in the queue`, icon_url: this.client.user.displayAvatarURL({ dynamic: !0, size: 512 }) },
                                  color: "#3A871F",
                                  fields: [
                                      {
                                          name: "Now playing",
        ...
        return (
            this.client.queue._waiting.find((t) => t.serverId === this.metadata.guild.id) &&
                this.client.queue._waiting
                    .filter((t) => t.serverId === this.metadata.guild.id)
                    .forEach((t) => {
                        this.client.queue.emitOp({ changes: ["NEXT_SONGS"], socketId: t.id, serverId: this.metadata.guild.id, queueData: { incoming: this.queue } }), this.client.queue.removeWaiting(t.id), this.client.queue._sockets.push(t);
                    }),
            this.client.queue._sockets.find((t) => t.serverId === this.metadata.guild.id) &&
                this.client.queue._sockets
                    .filter((t) => t.serverId === this.metadata.guild.id)
    ...
    addTrack(t, e, i) {
        return (
            t.info.requester || (t.info.requester = { name: e.username, id: e.id, avatar: e.displayAvatarURL({ dynamic: !0 }) }),
    ...
    async skip(t) {
        ...
        if (e.info.sp)
            if (e.info.uri && this.client.shoukaku.cache.find((t) => t.uri === e.info.uri)) e = this.client.shoukaku.cache.find((t) => t.uri === e.info.uri).data;
                ...
                const t = this.node;
                if (!e.info.author) {
                    const { getData: t } = require("spotify-url-info");
                    ...
                    e = { author: i.artists[0].name, title: i.name, url: e.info.uri, requester: e.info.requester };
                }
                let i = null;
                if (
                ...
                (i.info.title = e.title ? e.title : e.info.title),
                    ...
                    (e = i),
                    this.client.shoukaku.cache.find((t) => t.uri === i.info.uri) || this.client.shoukaku.cache.push({ uri: i.info.uri, data: i });
            }
        return this.previousTracks.push(this.current), (this.current = e), this.lastMessage && this.lastMessage.delete(), this.current ? this.player.playTrack(this.current.track, { noReplace: t || !1 }) && this.started() : this.skip(!0);
    }
    async play() {
    ...
    async handleAutoplay(t) {
        ...
        return s
            ? (this.previousTracks.find((t) => t.info.url === s.info.url) && (s = i.tracks[Math.floor(Math.random() * i.tracks.length)]),
              (s.info.requester = { name: "Autoplay System", id: "783708073390112830" }),
              this.queue.push(s),
              this.play())
            : this.destroy();
    }
        ...
        if (
            (this.client.queue._sockets.find((t) => t.serverId === this.metadata.guild.id) &&
                ...
                this.client.queue._sockets
                    .filter((t) => t.serverId === this.metadata.guild.id)
                    .forEach((e) => {
                        this.client.queue.emitOp({ changes: ["TRACK_REMOVED"], socketId: e.id, serverId: this.metadata.guild.id, queueData: { uri: isNaN(t) ? t.info.uri : this.queue[t].info.uri } });
                    }),
            isNaN(t))
        )
        ...
        return (
            this.client.queue._sockets.find((t) => t.serverId === this.metadata.guild.id) &&
                this.client.queue._sockets
                    .filter((t) => t.serverId === this.metadata.guild.id)
                    .forEach((t) => {
                        this.client.queue.emitOp({ changes: ["DESTROY"], socketId: t.id, serverId: this.metadata.guild.id, queueData: { current: null, incoming: [], recent: [] } }), this.client.queue.addWaiting(t);
                    }),
            t &&
                !this.metadata.message &&
                this.metadata.channel
                    .send({ embeds: [{ title: "Queue Concluded", color: "#F0B02F", description: "Queue has ended! Enjoying music with me? Consider [Voting for me](https://top.gg/bot/783708073390112830/vote)" }] })
                    .catch(() => null),
                    ...
                    embeds: [
                        {
                            ...
                            footer: { text: "Green-bot | Free music for everyone!", icon_url: this.client.user.displayAvatarURL({ dynamic: !0, size: 512 }) },
                            color: "#3A871F",
        ...
        this.metadata.channel
            ...
            this.client.queue._sockets.find((t) => t.serverId === this.metadata.guild.id) &&
                this.client.queue._sockets
                    .filter((t) => t.serverId === this.metadata.guild.id)
                    .forEach((t) => {
                        this.client.queue.emitOp({ changes: ["NEXT_SONGS", "CURRENT_SONG"], socketId: t.id, serverId: this.metadata.guild.id, queueData: { current: null, incoming: [] } });
                    });
    }
    destroy(t, e) {
        return (
