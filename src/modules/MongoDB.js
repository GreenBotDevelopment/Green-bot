...
class MongoDB {
    constructor(e) {
        (this.client = e),
            ...
            (this.blacklist = []),
            setInterval(() => {
                this.client.user.setActivity("*help - green-bot.app", { type: "WATCHING" }), (this.client.shoukaku.cache = []), (this.blacklist = []), this.client.sweepMessages(1800);
            }, 144e5);
    }
    async connect() {
        return (
    ...
    searchReq(e) {
        ...
        if (
            ...
            n > 26 &&
                ((t = !0),
                this.blacklist.push({ id: s, warns: 1 }),
                e.errorMessage(
            ...
            this.reqs.push({ id: s, timeout: e, requests: 1 });
        }
        return t;
    }
    async handleCache(e) {
    ...
    async getServer(e) {
        ...
        let s = await guildData.findOne({ serverID: e });
        return s ? (this.knowGuilds[e] = s) : (s = await new guildData({ serverID: e, prefix: "*", lang: "en", color: "#3A871F" }).save()), s;
    }
    async getUser(e, s) {
        let t = await user.findOne({ userID: e });
        return t || (t = await new user({ userID: e, playlists: [], songs: [] }).save()), t;
    }
    async suppr(e, s) {
        await guildData.findOneAndDelete({ serverID: e });
    }
