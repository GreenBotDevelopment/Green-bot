...
mongoose.connect(client.config.database.MongoURL, client.config.database.options).then(() => {
...
const init = async function() {
    ...
    client.player.on("trackAdd", async(queue, track) => {
        if (!queue.metadata || queue.metadata.controller) return console.log("Not metadata");
        const a = await queue.metadata.m.translate("MUSIC_ADDED", queue.metadata.guildDB.lang);
    ...
    client.player.on("queueEnd", async(queue, track) => {
        if (queue.metadata.controller) {
            ...
            const loadingTest = await queue.metadata.m.translate("QUEUE_END", queue.metadata.guildDB.lang)
...
init();
client.login(client.config.token).catch(e => { console.log("[Discord login]: Please provide a valid discord bot token\n" + e) });
