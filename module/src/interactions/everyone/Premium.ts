...
export default class Premium extends Command {
    ...
    async run({ ctx: e }) {
        ...
        const r = e.premiumlink("premiumUser") + "userId=" + e.member.id,
            ...
            a = await fetch(u).catch((t) => e.logger.error(t)),
            s = await a.json();
        ...
        if (t)
            if ("status" === t) {
                ...
                if ("activate" === t || "active" === t) {
                    ...
                    return (
                        await fetch(i, { method: "post", body: JSON.stringify(u), headers: { "Content-Type": "application/json" } }).catch((t) => e.logger.error(t)),
