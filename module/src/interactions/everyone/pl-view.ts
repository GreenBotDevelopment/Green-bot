...
export default class Queue extends Command {
    ...
    get arguments() {
        return [
            { name: "playlist_name", description: "The name of the playlist you want to see", required: true, type: 3 },
        ];
    }
    async run({ ctx: e }) {
        const t = e.args[0].value,
        n = await e.client.database.getUser(e.author.id);
    if ("liked-songs" === t) {
        if (!n.songs.length) return e.errorMessage("You don't have any liked song yet!");
        if (n.songs.length > 6) {
            let t = 0,
                o = 6,
                s = 1;
            ...
            e.client.collectors.create({
                ...
                exec: async (a) => {
                    await a.defer()
                    if (a.member.id !== e.author.id) return a.deleteOriginalMessage();
        ...
        if (!n || !n.playlists.find((e) => e.name === t)) return e.errorMessage("You don't have any playlist with this name!");
        const o = n.playlists.find((e) => e.name === t),
            s = o.tracks;
        if (s.length > 6) {
            let t = 0,
                n = 6,
            ...
            e.client.collectors.create({
                ...
                exec: async(u) => {
                    ...
                    if (u.member.id !== e.author.id) return u.deleteOriginalMessage();
