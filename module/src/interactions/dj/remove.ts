...
export default class Volume extends Command {
    ...
    get arguments() {
        return [{ type: 3, name: "track", description: "The position or the track of the track you want to remove", required: true }];
    }
    get checks() {
        return { voice: true, dispatcher: true, vote: false, channel: true, dj: true };
    }
    run({ ctx: e }) {
        let r = e.args[0].value.replace("#", "") - 1;
