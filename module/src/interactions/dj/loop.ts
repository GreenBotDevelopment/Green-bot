...
export default class Loop extends Command {
    ...
    run({ ctx: e }) {
        ...
        if ("queue" === a || "all" === a || "q" === a) {
            if ("queue" === e.dispatcher.repeat) return e.dispatcher.repeat, e.successMessage(":repeat_one: The loop mode has been disabled.");
            (e.dispatcher.repeat = "queue"), e.successMessage("🔄 Now looping the entiere queue.");
        } else if ("song" === a || "current" === a || "s" === a) {
            if ("song" === e.dispatcher.repeat) return e.dispatcher.repeat, e.successMessage(":repeat_one: The loop mode has been disabled.");
            (e.dispatcher.repeat = "song"), e.successMessage("🔂 Now looping the current track");
        } else {
            if ("disable" !== a && "off" !== a) return e.errorMessage("Please provide a valid option for this command! `queue`, `song` of `off`");
