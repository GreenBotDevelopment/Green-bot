...
export default class Repeat extends Command {
    ...
    run({ ctx: e }) {
        ...
        if ("queue" === o || "all" === o || "q" === o || e.message.content.includes("loopqueue")) {
            if ("queue" === e.dispatcher.repeat) return (e.dispatcher.repeat = "off"), e.successMessage(":repeat_one: The loop mode has been disabled.");
            ...
            if ("song" === e.dispatcher.repeat) return (e.dispatcher.repeat = "off"), e.successMessage(":repeat_one: The loop mode has been disabled.");
            (e.dispatcher.repeat = "song"), e.successMessage("🔂 Now looping the current track");
        } else {
            if ("disable" !== o && "off" !== o) return e.errorMessage("Please provide a valid option for this command! `queue`, `song` of `off`");
            if ("off" === e.dispatcher.repeat) return e.errorMessage("The loop mode is already disabled!");
