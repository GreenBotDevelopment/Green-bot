...
export default class Volume extends Command {
    ...
    async run({ ctx: e }) {
        if ("reset" === e.args[0]) return (e.guildDB.vcs = []), e.client.database.handleCache(e.guildDB), e.successMessage("The bot is now allowed to join every single voice channel!");
