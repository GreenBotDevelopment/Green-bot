...
export class CommandManager {
    ...
    async handle(message: Message<PossiblyUncachedTextableChannel>) {
        ...
        const data = await this.client.database.resolve(message.guildID)
        ...
        if (cleanedContent.startsWith(data.prefix.toLowerCase()) || cleanedContent.startsWith("<@!" + this.client.user.id + ">") || cleanedContent.startsWith("<@" + this.client.user.id + ">") || cleanedContent.startsWith("green")) {
            ...
            if (checks) {
                ...
                if (checks.voice === true && !member.voiceState.channelID) return context.errorMessage("You have to be connected in a voice channel before you can use this command!")
