module.exports = {
    ...
    async execute(message, args, client, guildDB) {
        ...
        if (prefix === "default" || prefix === "reset") prefix = client.config.prefix;
        if (prefix.length >= 5) return message.errorMessage(lang.err.replace("{prefix}", prefix));
        if (prefix.startsWith("[") && prefix.endsWith("]") || prefix.startsWith("<") && prefix.endsWith(">")) return message.errorMessage("Hooks such as `[]` or `<>` must not be used when executing commands. Ex: `" + guildDB.prefix + "setprefix !`.")
