...
const prompts = [
    "Bonjour ! commencez par me dire si je doit activer ou désactiver le système d'anti spam . \n Répondez par (**enable** ou **disable**) !",
    "Super ! et maintenant , à partir de quel nombre de messages par secondes je vais devoir punir ? Donnez juste un nombre entre 1 et 10. (Recommandé : 5)",
...
module.exports = {
        name: 'anti-spam',
        description: 'Active ou désactive le système contre le spam',
        ...
        async execute(message, args) {
            ...
            const verify = await Welcome.findOne({ serverID: message.guild.id, reason: `anti_spam` })
    ...
    message.reply({ embeds: [embed], allowedMentions: { repliedUser: false } }).then((m) => {
        ...
        const collector = m.createReactionCollector({ filter, time: 1000000});
            ...
            let b = await message.gg("le système d'anti spam")
            const verify = await Welcome.findOne({ serverID: message.guild.id, reason: `anti_spam` })
            if (verify) {
                const newchannel = await Welcome.findOneAndUpdate({ serverID: message.guild.id, reason: `anti_spam` }, { $set: { channelID: response.chances, reason: `anti_spam`, message: response.message,  image: response.percent, } }, { new: true });
...
message.channel.send(`**${okk}**`)
        ...
        if (i === 0) {
            ...
            if (content.toLowerCase() === 'enable') {
                ...
                const verify = await Welcome.findOne({ serverID: message.guild.id, reason: `anti_spam` })
                if (verify) {
                    const newchannel = await Welcome.findOneAndDelete({ serverID: message.guild.id, reason: `anti_spam` });
        ...
        if (i === 2) {
            if(content === 'default'){
                let a =await message.gg(`⚠ Attention {user} , tu envoies des messages trop vite  ! Si tu continues je serait obligé de te sanctionner...`)
                responses.message = a
            }else{
