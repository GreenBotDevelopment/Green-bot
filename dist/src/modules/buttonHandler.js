...
const SlashContext_1 = require("../modules/SlashContext");
class SlashManager {
    constructor(client) {
        this.client = client;
    }
    resolvePartials(message) {
        return __awaiter(this, void 0, void 0, function* () {
            ...
            channel = this.client.getChannel(message.channel.id) || (yield this.client.getRESTChannel(message.channel.id));
            member = message.member && message.member.username ? message.member : channel.guild && channel.guild.name ? channel.guild.members.get(message.member.id) ? channel.guild.members.get(message.member.id) : yield this.client.getRESTGuildMember(message.guildID, message.member.id) : yield this.client.getRESTGuildMember(message.guildID, message.member.id);
            guild = this.client.guilds.get(message.guildID) && this.client.guilds.get(message.guildID).name ? this.client.guilds.get(message.guildID) : channel.guild.name ? channel.guild : member.guild.name ? member.guild : yield this.client.getRESTGuild(message.guildID);
    ...
    handle(e) {
        return __awaiter(this, void 0, void 0, function* () {
            if (!this.client.cluster.ready && this.client.cluster.maintenance)
                return;
            if (this.client.collectors.handle(e, "button"))
                return;
            if (!e.acknowledged && e.data.custom_id !== "vote")
                yield e.defer();
            if (!e.guildID)
            ...
            const { channel, guild, member, me } = yield this.resolvePartials(e);
            if (e.type === 3) {
                const eligigle = this.client.shoukaku.checkEligible({ author: { id: e.member.id } });
                if (!eligigle && !(yield this.client.database.checkPremium(e.guildID, e.member.id, true))) {
                ...
                if (e.data && "back_queue" === e.data.custom_id || "next_queue" === e.data.custom_id)
                    return;
                if ("edit_pl" === e.data.custom_id) {
                    const t = yield this.client.database.getUser(e.member.id);
                    return void (t
                        ? e
                            .editOriginalMessage({
                            embeds: [
                            ...
                            let a, n = e.channel.createMessageCollector({ filter: (t) => t.author.id === e.member.id, time: 6e4 });
                            n.on("collect", (s) => __awaiter(this, void 0, void 0, function* () {
                ...
                if ("like" === e.data.custom_id) {
                    const s = yield this.client.database.getUser(e.member.id);
                    if (s.songs.find((e) => e.info.uri === t.current.info.uri))
                        return ((s.songs = s.songs.filter((e) => e.info.uri !== t.current.info.uri)),
                            s.save(),
                            e
                                .editOriginalMessage({
                                embeds: [
                ...
                if (!e.member.voiceState.channelID || e.member.voiceState.channelID !== me.voiceState.channelID)
                    return e
                        ...
                        .catch((e) => { });
                const s = yield this.client.database.resolve(e.guildID);
                if ("save_pl" === e.data.custom_id) {
                    if (0 == t.queue.length)
                        return e
                            ...
                            .catch((e) => { });
                    let s = yield this.client.database.getUser(e.member.id), a = `${guild.name}'s queue | #0`;
                    if (s.playlists.find((e) => e.name === a)) {
                        const t = s.playlists.find((e) => e.name === a).name, n = Number(t.charAt(t.length - 1));
                ...
                if ("shuffle" === e.data.custom_id) {
                    if (t.queue.length < 2)
                        return e.editOriginalMessage({ embeds: [{ description: "There are no enough tracks in the queue to shuffle it!", color: 0xC73829 }] }).catch((e) => { });
                    ...
                    channel && channel.voiceMembers.filter(m => !m.bot).length >= 2 && s.vote_skip
                        ? t && t.voting
                            ...
                            : e
                                ...
                                t.voting = true;
                                let s = 0, a = [], n = Math.round((channel.voiceMembers.filter(m => !m.bot).length + 1) / 2);
                                this.client.collectors.create({
                                    channelId: e.channel.id,
                                    time: 60000,
                                    type: "button",
                                    filter: (e) => "vote" === e.data.custom_id,
                                    exec: (r) => __awaiter(this, void 0, void 0, function* () {
                ...
                try {
                    ...
                    const s = yield this.client.database.resolve(e.guildID), a = new SlashContext_1.SlashContext(this.client, e, e.data.options || [], s, me, member);
                    if (this.client.config.premiumCmd.includes(t.name) && !(yield this.client.database.checkPremium(e.guildID, e.member.id, true))) {
                        return a.reply({
                            embeds: [
                                {
                                    color: 0x3A871F,
                    ...
                    if (this.client.config.voteLocks.includes(t.name)) {
                        const t = yield this.client.database.checkPremium(e.guildID, e.member.id, true), s = yield this.checkVoted(e.member.id);
                        if (!t && !s)
                            return e.editOriginalMessage({
                                embeds: [
                                    {
                    ...
                    return this.client.hasBotPerm(a, "embedLinks")
                        ? s.txts && 0 !== s.txts.length && !s.txts.includes(`${e.channel.id}`) && "textchannels" !== t.name
                            ? a.errorMessage("I am not allowed to answer to commands in this channel.\n" +
                                    ...
                                    : (t.checks && t.checks.dispatcher && !a.dispatcher) || (t.checks && t.checks.dispatcher && a.dispatcher && !a.dispatcher.playing)
                                        ...
                                        : a.guildDB.dj_commands && a.guildDB.dj_commands.includes(t.name) && !this.checkDJC(a)
                                            ...
                                            : t.checks && t.checks.channel && member.voiceState.channelID && me.voiceState.channelID && member.voiceState.channelID !== me.voiceState.channelID
                                                ? a.errorMessage("You need to be in the same voice channel as me (<#" +
    ...
    checkVoted(e) {
        return __awaiter(this, void 0, void 0, function* () {
            let t = null;
            try {
                t = yield this.client.dbl.hasVoted(e);
            }
            catch (e) {
    ...
    checkDJC(context) {
        ...
        if (!context.guildDB.djroles || !context.guildDB.djroles.length)
            isDj = true;
        if (context.guildDB.djroles && context.guildDB.djroles.length && context.member.roles.find(r => context.guildDB.djroles.includes(r)))
            isDj = true;
