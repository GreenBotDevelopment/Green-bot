...
class Pause extends QuickCommand_1.Command {
    get name() {
        return "resume";
    }
    get description() {
    ...
    run({ ctx: e }) {
        return (e.client.queue._sockets.find((s) => s.serverId === e.guild.id) &&
            e.client.queue._sockets
                .filter((s) => s.serverId === e.guild.id)
                .forEach((s) => {
                e.client.queue.emitOp({
                    changes: ["CURRENT_SONG", "RECENT_SONGS"],
                    socketId: s.id,
                    serverId: e.guild.id,
                    queueData: { current: e.dispatcher.current, paused: e.dispatcher.player.paused, loop: "queue" === e.dispatcher.repeat },
                });
            }),
            ...
            e.dispatcher.player.paused ? (e.dispatcher.pause(false), e.successMessage("▶ Music Unpaused!")) : e.errorMessage("The player is not paused so I can't unpause it!"));
    }
}
