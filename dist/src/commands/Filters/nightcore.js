...
class Nightcore extends QuickCommand_1.Command {
    ...
    run({ ctx: e }) {
        return __awaiter(this, void 0, void 0, function* () { return e.dispatcher.filters.includes("nightcore") ? (e.dispatcher.player.setTimescale(), e.dispatcher.filters = e.dispatcher.filters.filter(e => "nightcore" !== e), e.successMessage("⏱ Disabling the `nightcore` filter to the current song...")) : (e.dispatcher.player.setTimescale({ speed: 1.165, pitch: 1.125, rate: 1.05 }), e.dispatcher.filters.push("nightcore"), e.successMessage("⏱ Enabling the `nightcore` filter to the current song...")); });
    }
}
