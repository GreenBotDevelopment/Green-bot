    ...
    async handle(e) {
        ...
        if (e.type === 3) {
            ...
            if ("save_pl" === e.data.custom_id) {
                if (0 == t.queue.length)
                    return e
                        ...
                        .catch((e) => { });
                let s = await this.client.database.getUser(e.member.id), a = `${guild.name}'s queue | #0`;
            ...
            try {
                ...
                const s = await this.client.database.resolve(e.guildID), a = new SlashContext_1.SlashContext(this.client, e, e.data.options || [], s, me, member);
                ...
                if (this.client.config.voteLocks.includes(t.name)) {
                    const t = await this.client.database.checkPremium(e.guildID, e.member.id, true), s = await this.checkVoted(e.member.id);
                    if (!t && !s)
                        return e.editOriginalMessage({
                            embeds: [
                                {
