    ...
    async startServer(e) {
        ...
        r.use((0, body_parser_1.urlencoded)({ extended: false })),
            ...
            r.get("/fetchserver", async function (r, s) {
                try {
                    const t = r.query.server, a = r.query.asking, n = (await e.cluster.broadcastEval(async (e, r) => {
                        const s = e.guilds.get(r.serv);
                        if (!s)
                            return null;
                        let t = await e.database.resolve(s.id), a = e.queue.get(s.id), n = false, u = s.members.get(r.user) || await s.getRESTMember(r.user), o = await e.database.getUser(r.user), c = true, l = await e.database.checkPremium(r.serv, r.user);
            ...
            r.get("/removeSong", async function (r, s) {
                ...
                await e.cluster.broadcastEval(async (e, r) => {
                    const s = e.guilds.get(r.serv);
                    if (!s)
                        return;
            ...
            r.post("/remove_pl_track", async function (r, s) {
                const { song: t, playlist: a } = r.body, n = await e.database.getUser(r.query.user);
                if ("Liked Song" === a)
            ...
            r.post("/handle_like", async function (r, s) {
                const { song: t } = r.body, a = await e.database.getUser(r.query.user);
            ...
            r.get("/addServer", async function (r, s) {
                ...
                e.cluster.broadcastEval(async (e, r) => {
                    const s = e.guilds.get(r.serv);
            ...
            r.get("/exists", async function (r, s) {
                try {
                    const t = r.query.server, a = (await e.cluster.broadcastEval(async (e, r) => {
            ...
            r.get("/on_db", async function (r, s) {
                try {
                    const t = r.query.server, a = await e.client.database.resolve(t);
            ...
            r.get("/del_db", async function (r, s) {
                try {
                    const t = r.query.server, a = await e.client.database.suppr(t);
                    s.send(a ? { exists: true } : { exists: false });
                }
                catch (e) {
                    return s.send({ code: 3 });
