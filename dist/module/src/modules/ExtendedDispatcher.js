...
class ExtendedDispatcher {
    ...
    constructor(client, metadata, player, node) {
        ...
        this.player.on("exception", (event) => {
            this.errored = "no";
            if (this.debug)
                console.log(`[Dispatcher => ${this.metadata.guild.id}] Track failed to play ( ${this.current?.info.title} ) for reason: ${event.error}. Cause: ${event.exception?.cause}`);
            ...
            if (this.client.shoukaku.cache.find(item => item.id === nextTrack.info.uri)) {
                ...
                if (!nextTrack.info.author) {
                    if (!nextTrack.info.uri)
                        return this.skip(true);
                    const scrapedData = await (0, spotify_url_info_1.getData)(nextTrack.info.uri);
                    if (!scrapedData)
                        return this.skip(true);
