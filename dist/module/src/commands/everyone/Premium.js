...
class Premium extends QuickCommand_1.Command {
    ...
    async run({ ctx: e }) {
        const t = e.args[0];
        if (t) {
            const r = e.premiumlink("premiumUser") + "userId=" + e.member.id, i = await fetch(r).catch((e) => console.error(e)), o = await i.json(), s = e.premiumlink("premiumGuild") + "guildId=" + e.guild.id, n = await fetch(s).catch((e) => console.error(e)), a = await n.json();
            ...
            if (t)
                if ("status" === t) {
                    ...
                    if ("activate" === t || "active" === t) {
                        ...
                        return (await fetch(i, { method: "post", body: JSON.stringify(s), headers: { "Content-Type": "application/json" } }).catch((e) => console.error(e)),
