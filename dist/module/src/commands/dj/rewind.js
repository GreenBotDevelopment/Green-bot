...
class Volume extends QuickCommand_1.Command {
    ...
    run({ ctx: e }) {
        let s = e.args[0] || "10s";
        if (!s.includes("m") && !s.includes("s") && !s.includes("h"))
            s = s + "s";
        const time = ms(s);
        if (!time || isNaN(time) || time < 0)
            return e.errorMessage("The duration you provided is incorrect. Please provide a number of seconds.");
        if (time > e.dispatcher.current.info.length)
